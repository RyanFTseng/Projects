...
def receive_file(window):
        global recvtxt
        recvtxt=s.recv(1024)
        if recvtxt!=b'':
                master.filename=filedialog.asksaveasfilename(initialdir='/home/pi/ywryantseng',title='select file')
                recvtxt=recvtxt.decode()
                print('recieved message:'+recvtxt)
                f=open(master.filename,'w+')
                f.write(recvtxt)
                addtowindow(master.filename,window)
